  ...
  return (
    <main className="min-h-screen font-sans flex flex-col items-center justify-between bg-gradient-to-br from-[#222] to-[#111] text-white relative overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center px-4 sm:px-6 relative z-10 w-full">
        <div className="bg-gradient-to-br from-[#111]/80 to-[#222]/80 backdrop-blur-xl p-6 sm:p-8 lg:p-10 rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-lg border border-pink-500/20 relative">
          {/* Decoração interna */}
          ...
          <div className="relative z-10">
              ...
              <div>
                ...
                <form onSubmit={handleSendResetCode} className="space-y-4 sm:space-y-6">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-3 sm:py-4 px-4 sm:px-6 rounded-lg sm:rounded-xl shadow-lg transition-all transform hover:scale-105 hover:shadow-xl text-sm sm:text-base"
                  >
            ...
            {step === "code" && (
              <div>
                <p className="text-gray-300 text-center mb-6 sm:mb-8 leading-relaxed text-sm sm:text-base">
                  ...
                  <strong className="text-pink-400 text-base sm:text-lg">{email}</strong>
                </p>
                <form onSubmit={handleVerifyCode} className="space-y-4 sm:space-y-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full p-4 sm:p-6 bg-[#222] border-2 border-pink-500/30 rounded-lg sm:rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-pink-500/60 focus:ring-2 focus:ring-pink-500/20 transition-all text-center tracking-[0.5em] sm:tracking-[0.8em] text-xl sm:text-2xl font-mono"
                      maxLength={6}
                      required
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-3 sm:py-4 px-4 sm:px-6 rounded-lg sm:rounded-xl shadow-lg transition-all transform hover:scale-105 hover:shadow-xl text-sm sm:text-base"
                  >
                    <span className="flex items-center justify-center space-x-2">
                      {loading ? (
                        <>
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-2.5 sm:py-3 px-4 sm:px-6 rounded-lg sm:rounded-xl shadow-lg transition-all transform hover:scale-105 text-sm sm:text-base"
                  >
                    <span className="flex items-center justify-center space-x-2">
                ...
                <button
                  ...
                  className="w-full mt-4 sm:mt-6 p-2 text-gray-400 hover:text-pink-400 transition-colors text-xs sm:text-sm font-medium"
                >
                  ← Alterar email
                </button>
              </div>
                ...
                <form onSubmit={handleResetPassword} className="space-y-4 sm:space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className={`w-full p-3 sm:p-4 rounded-lg sm:rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 transition-all border-2 text-sm sm:text-base ${
                    ...
                    />
                    {confirmPassword && newPassword !== confirmPassword && (
                      <p className="text-red-400 text-xs sm:text-sm font-medium">❌ As senhas não coincidem</p>
                    )}
                    {confirmPassword && newPassword === confirmPassword && confirmPassword.length >= 6 && (
                      <p className="text-green-400 text-xs sm:text-sm font-medium">✅ Senhas coincidem</p>
                    )}
                  </div>
            ...
            {message && (
              <div className={`mt-6 sm:mt-8 p-3 sm:p-4 rounded-lg sm:rounded-xl text-center font-medium border-2 backdrop-blur-sm text-sm sm:text-base ${
            ...
            <div className="text-center mt-6 sm:mt-8">
              <div className="flex items-center gap-4 mb-4 sm:mb-6">
                ...
                <span className="text-gray-400 text-xs sm:text-sm">voltar</span>
                <div className="flex-1 h-px bg-gradient-to-r from-transparent via-gray-600 to-transparent"></div>
              </div>
              
              <button
                onClick={() => router.push("/login")}
                className="text-pink-400 hover:text-pink-300 font-semibold transition-colors flex items-center justify-center space-x-2 mx-auto text-sm sm:text-base"
              >
                <span>←</span>
